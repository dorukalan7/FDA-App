...
class Radiationpage extends StatelessWidget {
  ...
  const Radiationpage({required this.title, required this.imageUrl, super.key});

  void navigateToMedicalDevicePage(BuildContext context, String status) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => MedicalDevicePage(title: title, status: status),
      ),
    );
  }
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16),
                child: Boxx(
                  label: 'Radiation Products',
                  onTap: () => navigateToMedicalDevicePage(context, 'APPR'),
                ),
              ),
            ],
          ),
